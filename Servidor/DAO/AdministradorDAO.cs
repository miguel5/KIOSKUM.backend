        ...
        public void EditarConta(Administrador administradorEditado)
        {
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("?id", administradorEditado.IdFuncionario);
                    cmd.Parameters["?id"].Direction = ParameterDirection.Input;

                    cmd.Parameters.AddWithValue("?nome", administradorEditado.Nome);
                    cmd.Parameters["?nome"].Direction = ParameterDirection.Input;

                    cmd.Parameters.AddWithValue("?numero", administradorEditado.NumFuncionario);
                    cmd.Parameters["?numero"].Direction = ParameterDirection.Input;

                    cmd.Parameters.AddWithValue("?pass", administradorEditado.Password);
            ...
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    ...
                    using (MySqlDataReader var = cmd.ExecuteReader())
                    {
                        ...
                        if (var.Read())
                        {
                            administrador = new Administrador { IdFuncionario = idFuncionario, Nome = var.GetString(0), NumFuncionario = var.GetInt32(2), Password = var.GetString(1) };
                        }
